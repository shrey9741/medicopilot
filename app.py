...
Design: "The Clinical Luminary" — with functional top nav tabs
"""
...
.stButton > button {
    ...
    color: #a9abb3 !important; border: none !important; border-radius: 8px !important;
...
.stButton > button:hover { color: #ecedf6 !important; background: rgba(255,255,255,0.05) !important; }

/* Run button override — target by key via parent div */
div[data-testid="stVerticalBlock"] div.run-btn .stButton > button {
...
.top-bar {
    ...
    padding: 4px 12px 0; margin-bottom: 20px;
...
.ai-badge {
    ...
    border-radius: 20px; padding: 5px 14px; margin-bottom: 4px;
...
.panel-title {
    ...
    color: #ecedf6; margin-bottom: 16px; display: flex; align-items: center; gap: 8px;
}
.model-badge {
    background: rgba(105,218,255,0.08); border: 1px solid rgba(105,218,255,0.2);
...
.section-header { font-family:'Manrope',sans-serif; font-size:1rem; font-weight:800; color:#ecedf6; margin-bottom:14px; padding-bottom:10px; border-bottom:1px solid rgba(255,255,255,0.06); }
...
for i, tab in enumerate(TOP_TABS):
    with nav_cols[i]:
        ...
        label = f"**:blue[{tab}]**" if is_active else tab
        if st.button(label, key=f"nav_{tab}"):
            st.session_state.top_tab = tab
            st.rerun()
with nav_cols[4]:
    st.markdown('<div style="display:flex;justify-content:flex-end;padding:4px 0"><div class="ai-badge"><span class="ai-dot"></span>AI CLINICAL COPILOT</div></div>', unsafe_allow_html=True)
...
with left_col:
    ...
    run_btn = st.button("⚡  Run AI Analysis", use_container_width=True, key="run_analysis")
    st.markdown('</div>', unsafe_allow_html=True)

    # Override run button style
    st.markdown("""
    <style>
...
with center_col:
    if not data:
        ...
        # ── TAB: Multi-Agent ──────────────────────────────────────────────────
            ...
            st.markdown('<div style="font-size:0.78rem;color:#a9abb3;margin-bottom:16px">How all 9 agents collaborated to produce this clinical briefing</div>', unsafe_allow_html=True)
            for i, step in enumerate(data.get("reasoning_trace", [])):
                num = f"0{i+1}" if i < 9 else str(i+1)
                agent_colors = {
                ...
                <div class="trace-card" style="border-left:3px solid {color}">
                    <div class="trace-num" style="color:{color};background:transparent;border:1px solid {color}22">{num}</div>
                    <div style="flex:1">
        ...
        # ── TAB: Drug Safety ─────────────────────────────────────────────────
            ...
            if not warnings:
                ...
                sev_classes = {"critical":"sev-crit","major":"sev-major","moderate":"sev-moderate","minor":"sev-minor"}
                for w in warnings:
                    icon = icons.get(w["severity"],"⚪")
                    sc = sev_classes.get(w["severity"],"")
                    st.markdown(f"""
                    <div class="std-card">
                        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
                            <span style="font-size:1rem">{icon}</span>
                            <div class="std-title" style="margin-bottom:0">{w['drug_a']} + {w['drug_b']}</div>
                            <span class="{sc}" style="font-size:0.68rem;margin-left:auto">{w['severity'].upper()}</span>
                        </div>
                        <div class="std-text">{w['recommendation']}</div>
                    </div>""", unsafe_allow_html=True)

            # Also show second opinion related to drug safety
            if data.get("second_opinion"):
                op = data["second_opinion"]
                st.markdown(f"""
        ...
        # ── TAB: SOAP Notes ───────────────────────────────────────────────────
            ...
            st.markdown('<div style="font-size:0.78rem;color:#a9abb3;margin-bottom:16px">AI-generated structured clinical documentation ready for EHR entry</div>', unsafe_allow_html=True)
            soap = data.get("soap_note", {})
            for label, content, emoji_icon in [
                ...
                st.markdown(f"""
                <div class="soap-card">
                    <div class="soap-label">{emoji_icon} {label}</div>
...
with right_col:
    ...
    <div class="right-panel-box">
        <div class="rp-title">Recent Analyses <span style="font-size:16px;color:#a9abb3">🕐</span></div>
    """, unsafe_allow_html=True)
