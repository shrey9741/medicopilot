...
/* All buttons transparent by default */
.stButton > button {
    background: transparent !important; color: #a9abb3 !important;
    border: none !important; border-radius: 0 !important;
    padding: 8px 4px !important; font-size: 0.82rem !important; font-weight: 500 !important;
    width: 100% !important; box-shadow: none !important; transition: color 0.2s !important;
    border-bottom: 2px solid transparent !important;
}
.stButton > button:hover { color: #ecedf6 !important; background: transparent !important; }

/* Run button */
div.run-btn .stButton > button {
    background: linear-gradient(135deg, #69daff 0%, #17c0fd 100%) !important;
    color: #003d4f !important; border-radius: 12px !important;
    ...
    box-shadow: 0 4px 20px rgba(105,218,255,0.25) !important;
    width: 100% !important; border: none !important; border-bottom: none !important;
}

/* Export button */
div.export-btn .stButton > button {
    background: #161a21 !important; color: #69daff !important;
    border: 1px solid rgba(105,218,255,0.2) !important; border-radius: 10px !important;
    padding: 8px 16px !important; font-size: 0.8rem !important; font-weight: 600 !important;
    width: auto !important; border-bottom: 1px solid rgba(105,218,255,0.2) !important;
}
...
.top-bar {
    ...
    border: 1px solid rgba(255,255,255,0.05); height: 48px;
}
.ai-badge {
    margin-left: auto; display: flex; align-items: center; gap: 6px;
    background: rgba(105,218,255,0.08); border: 1px solid rgba(105,218,255,0.2);
    border-radius: 20px; padding: 5px 14px; align-self: center;
    font-size: 0.68rem; font-weight: 700; color: #69daff; letter-spacing: 0.08em; white-space: nowrap;
}
.ai-dot { width: 6px; height: 6px; background: #69daff; border-radius: 50%; display: inline-block; margin-right: 4px; }

.left-panel-box { background: #161a21; border-radius: 16px; border: 1px solid rgba(255,255,255,0.05); padding: 20px; }
.panel-title { font-family: 'Manrope', sans-serif; font-size: 1rem; font-weight: 800; color: #ecedf6; margin-bottom: 16px; display: flex; align-items: center; gap: 8px; }
.model-badge { background: rgba(105,218,255,0.08); border: 1px solid rgba(105,218,255,0.2); border-radius: 20px; padding: 2px 8px; font-size: 0.6rem; font-weight: 700; color: #69daff; }
.section-label { font-size: 0.62rem; font-weight: 700; letter-spacing: 0.15em; text-transform: uppercase; color: #a9abb3; margin-bottom: 8px; margin-top: 16px; }
.agents-wrap { display: flex; flex-wrap: wrap; gap: 5px; margin-top: 6px; }
.agent-pill { background: #1c2028; border-radius: 8px; padding: 3px 9px; font-size: 0.66rem; font-weight: 600; color: #17c0fd; }

.glass-card { background: rgba(34,38,47,0.5); backdrop-filter: blur(12px); border: 1px solid rgba(105,218,255,0.15); border-radius: 16px; padding: 24px; margin-bottom: 14px; box-shadow: 0 20px 40px rgba(0,209,255,0.06); position: relative; overflow: hidden; }
.glass-card::after { content:''; position:absolute; top:-30px; right:-30px; width:100px; height:100px; background:rgba(105,218,255,0.05); filter:blur(25px); border-radius:50%; pointer-events:none; }
.card-title { font-family:'Manrope',sans-serif; font-size:1.05rem; font-weight:800; color:#ecedf6; display:flex; align-items:center; gap:10px; margin-bottom:4px; }
.conf-badge { background:rgba(105,218,255,0.1); border:1px solid rgba(105,218,255,0.25); border-radius:20px; padding:2px 8px; font-size:0.62rem; font-weight:900; color:#69daff; letter-spacing:0.08em; }
.card-sub { font-size:0.75rem; color:#a9abb3; margin-bottom:16px; }

.dx-inner { background:rgba(0,0,0,0.35); border:1px solid rgba(255,255,255,0.05); border-radius:12px; padding:18px; margin-bottom:12px; }
...
.conflict-bar { background:rgba(28,32,40,0.6); border:1px solid rgba(255,255,255,0.05); border-radius:12px; padding:12px 16px; display:flex; align-items:center; gap:12px; margin-bottom:12px; }
...
.section-header { font-family:'Manrope',sans-serif; font-size:1rem; font-weight:800; color:#ecedf6; margin-bottom:6px; }
.section-sub { font-size:0.76rem; color:#a9abb3; margin-bottom:16px; }
...
.history-card { background:#10131a; border:1px solid rgba(255,255,255,0.04); border-radius:12px; padding:12px; margin-bottom:8px; transition:all 0.2s; }
...
TOP_TABS = ["RAG", "Multi-Agent", "Drug Safety", "SOAP Notes"]

# ── Top nav bar ───────────────────────────────────────────────────────────────
nav_cols = st.columns([1, 1, 1, 1, 4])
for i, tab in enumerate(TOP_TABS):
    with nav_cols[i]:
        is_active = st.session_state.top_tab == tab
        # Active tab gets blue underline via markdown styling
        if st.button(tab, key=f"nav_{tab}", use_container_width=True):
            st.session_state.top_tab = tab
            st.rerun()

# Inject active tab underline dynamically
active_idx = TOP_TABS.index(st.session_state.top_tab)
st.markdown(f"""
<style>
div[data-testid="column"]:nth-child({active_idx + 1}) .stButton > button {{
    color: #69daff !important;
    border-bottom: 2px solid #69daff !important;
}}
</style>
""", unsafe_allow_html=True)

# AI badge + top bar background
st.markdown(f"""
<style>
/* Wrap nav in styled bar */
section.main > div > div:nth-child(1) {{
    background: #161a21;
    border-radius: 14px;
...
<div style="display:flex;justify-content:flex-end;margin-top:-44px;margin-bottom:12px;padding-right:8px">
...
with left_col:
    ...
    run_btn = st.button("⚡  Run AI Analysis", use_container_width=True, key="run_btn")
    ...
    div.run-btn .stButton > button {
        ...
        font-weight: 800 !important; font-size: 0.88rem !important;
        box-shadow: 0 4px 20px rgba(105,218,255,0.25) !important;
        border: none !important; border-bottom: none !important; width: 100% !important;
    }
...
if run_btn:
    ...
    with center_col:
        ...
        @keyframes agentpulse { 0%,100%{opacity:0.4;transform:scale(0.95)} 50%{opacity:1;transform:scale(1)} }
        .loading-card {
        ...
        .loading-title { font-family:'Manrope',sans-serif; font-size:1.2rem; font-weight:800; color:#ecedf6; margin-bottom:8px; }
        .loading-sub { font-size:0.78rem; color:#a9abb3; margin-bottom:28px; }
        .agent-seq { display:flex; justify-content:center; gap:8px; flex-wrap:wrap; margin-bottom:28px; }
        .agent-seq span { background:#1c2028; border-radius:8px; padding:5px 12px; font-size:0.68rem; font-weight:600; color:#69daff; display:flex; align-items:center; gap:5px; }
        .agent-seq span.a1 { animation: agentpulse 2s 0.0s infinite; color:#69daff; }
        .agent-seq span.a2 { animation: agentpulse 2s 0.2s infinite; color:#17c0fd; }
        .agent-seq span.a3 { animation: agentpulse 2s 0.4s infinite; color:#ff716c; }
        .agent-seq span.a4 { animation: agentpulse 2s 0.6s infinite; color:#89a5ff; }
        .agent-seq span.a5 { animation: agentpulse 2s 0.8s infinite; color:#f97316; }
        .agent-seq span.a6 { animation: agentpulse 2s 1.0s infinite; color:#f59e0b; }
        .agent-seq span.a7 { animation: agentpulse 2s 1.2s infinite; color:#10b981; }
        .agent-seq span.a8 { animation: agentpulse 2s 1.4s infinite; color:#a9abb3; }
        .agent-seq span.a9 { animation: agentpulse 2s 1.6s infinite; color:#69daff; }
        .progress-track { width:100%; height:6px; background:#161a21; border-radius:20px; overflow:hidden; position:relative; margin-bottom:16px; }
        .progress-bar { position:absolute; height:100%; width:40%; background:linear-gradient(90deg,transparent,#17c0fd,#69daff,transparent); border-radius:20px; animation: shimmer 1.8s ease-in-out infinite; }
        .loading-hint { font-size:0.7rem; color:#45484f; font-family:'Inter',sans-serif; }
        </style>
        <div class="loading-card">
            <div style="font-size:2.8rem;margin-bottom:16px">🧠</div>
            <div class="loading-title">Generating Clinical Briefing...</div>
            <div class="loading-sub">9 AI agents are collaborating on your patient analysis</div>
            <div class="agent-seq">
                <span class="a1">🏥 FHIR</span>
                <span class="a2">💾 Memory</span>
                <span class="a3">⚡ Anomaly</span>
                <span class="a4">📚 RAG</span>
                <span class="a5">🔬 Diagnosis</span>
                <span class="a6">💊 Drug Safety</span>
                <span class="a7">📊 Risk</span>
                <span class="a8">🧐 2nd Opinion</span>
                <span class="a9">📝 SOAP</span>
            </div>
            <div class="progress-track"><div class="progress-bar"></div></div>
            <div class="loading-hint">This may take 20–40 seconds on first request</div>
        </div>
        """, unsafe_allow_html=True)
...
top_tab = st.session_state.top_tab
...
with center_col:
    if not data:
        ...
        # ── Always shown: anomaly + patient card ──────────────────────────────
        ...
        # ── RAG tab ───────────────────────────────────────────────────────────
        if top_tab == "RAG":
            t1, t2, t3 = st.tabs(["🔬 Diagnosis", "📊 Risk Scores", "🔍 Reasoning"])
            ...
            with t1:
                ...
                if diagnoses:
                    ...
                    if warnings:
                        ...
                        sev_label = "HIGH" if sev in ["critical","major"] else sev.title()
                        st.markdown(f"""
                        <div class="conflict-bar">
                            <div style="width:38px;height:38px;background:#006688;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">🛡️</div>
                            <div>
                                <div class="conflict-title">Drug-Drug Conflict Detected</div>
                                <div class="conflict-sub">{w['drug_a']} + {w['drug_b']}</div>
                            </div>
                            <div class="conflict-sev">{sev_label}</div>
                        </div>""", unsafe_allow_html=True)
                ...
                if data.get("second_opinion"):
                    ...
                    <div class="opinion-card">
                        <div class="opinion-label">🧐 Second Opinion</div>
                        ...
                        <div style="font-size:0.76rem;color:#69daff"><strong>Recommendation:</strong> {op['final_recommendation']}</div>
                    </div>""", unsafe_allow_html=True)
        ...
        # ── Multi-Agent tab ───────────────────────────────────────────────────
        elif top_tab == "Multi-Agent":
            st.markdown('<div class="section-header">Multi-Agent Reasoning Trace</div>', unsafe_allow_html=True)
            st.markdown('<div class="section-sub">How all 9 agents collaborated to produce this briefing</div>', unsafe_allow_html=True)
            agent_colors = {
                "FHIRAgent": "#69daff", "MemoryAgent": "#89a5ff",
                "AnomalyDetector": "#ff716c", "RAGAgent": "#17c0fd",
                "DiagnosisAgent": "#69daff", "DrugSafetyAgent": "#f97316",
                "RiskScoringAgent": "#f59e0b", "SecondOpinionAgent": "#10b981",
                "SOAPNoteGenerator": "#a9abb3",
            }
            for i, step in enumerate(data.get("reasoning_trace", [])):
                num = f"0{i+1}" if i < 9 else str(i+1)
                color = agent_colors.get(step['agent'], '#69daff')
                st.markdown(f"""
                <div class="trace-card" style="border-left:3px solid {color}">
                    <div class="trace-num" style="color:{color};background:transparent;border:1px solid {color}44">{num}</div>
                    <div style="flex:1">
                        <div class="trace-agent" style="color:{color}">{step['agent']}</div>
                        <div class="trace-action">{step['action']}</div>
                        <div class="trace-finding" style="color:{color}99">{step['finding']}</div>
                    </div>
                </div>""", unsafe_allow_html=True)

            if data.get("memory_trend"):
                st.markdown('<div class="section-header" style="margin-top:20px">📈 Patient Memory Trends</div>', unsafe_allow_html=True)
                st.code(data["memory_trend"], language="text")

        # ── Drug Safety tab ───────────────────────────────────────────────────
        elif top_tab == "Drug Safety":
            st.markdown('<div class="section-header">Drug Interaction Matrix</div>', unsafe_allow_html=True)
            st.markdown('<div class="section-sub">AI-detected drug interactions and clinical recommendations</div>', unsafe_allow_html=True)
            warnings = data.get("drug_warnings", [])
            if not warnings:
                ...
                sev_colors = {"critical":"#ff716c","major":"#f97316","moderate":"#f59e0b","minor":"#69daff"}
                for w in warnings:
                    icon = icons.get(w["severity"],"⚪")
                    sc = sev_colors.get(w["severity"],"#a9abb3")
                    st.markdown(f"""
                    <div class="std-card">
                        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
                            <span style="font-size:1.1rem">{icon}</span>
                            <div class="std-title" style="margin-bottom:0">{w['drug_a']} + {w['drug_b']}</div>
                            <span style="font-size:0.68rem;font-weight:700;color:{sc};margin-left:auto;background:{sc}18;border:1px solid {sc}44;border-radius:6px;padding:2px 8px">{w['severity'].upper()}</span>
                        </div>
            ...
            if data.get("second_opinion"):
                op = data["second_opinion"]
                st.markdown(f"""
                <div class="opinion-card" style="margin-top:16px">
                    <div class="opinion-label">🧐 Clinical Safety Second Opinion</div>
                    <div style="font-size:0.76rem;color:#a9abb3;margin-bottom:7px"><strong style="color:#ecedf6">Challenge:</strong> {op['challenge']}</div>
                    <div style="font-size:0.76rem;color:#69daff"><strong>Recommendation:</strong> {op['final_recommendation']}</div>
                </div>""", unsafe_allow_html=True)

        # ── SOAP Notes tab ────────────────────────────────────────────────────
        elif top_tab == "SOAP Notes":
            st.markdown('<div class="section-header">SOAP Clinical Note</div>', unsafe_allow_html=True)
            st.markdown('<div class="section-sub">AI-generated structured clinical documentation ready for EHR entry</div>', unsafe_allow_html=True)
            soap = data.get("soap_note", {})
            for label, content, emo in [
...
with right_col:
    ...
    <div class="right-panel-box">
        <div class="rp-title">Recent Analyses <span style="font-size:14px;color:#a9abb3">🕐</span></div>
    """, unsafe_allow_html=True)
    history = [
