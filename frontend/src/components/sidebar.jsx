...
export default function Sidebar({ activePatientId }) {
  ...
  const navItems = [
    ...
    { label: 'Patient Briefing', icon: 'assignment_ind', path: `/patient/${activePatientId || 'P001'}` },
    { label: 'SOAP Generator', icon: 'history_edu', path: '/soap' },
    { label: 'Agent Status', icon: 'smart_toy', path: '/agents' },
  ];

  const isActive = (path) => {
    if (path.startsWith('/patient/')) return location.pathname.startsWith('/patient/');
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '24px 20px 20px' }}>
        ...
        }}>Clinical Sentinel</div>
        <div style={{
          fontSize: '9px', fontWeight: 700, letterSpacing: '0.14em',
          textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)',
          marginTop: '3px',
        }}>Medical AI Copilot</div>
      </div>
